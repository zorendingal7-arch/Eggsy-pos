...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $conn->query("INSERT INTO menu_items (name, category_id, price, image, subcategory, is_available, created_at)
            VALUES ('$name', $category_id, $price, $img_sql, $sub_sql, 1, NOW())");
        ...
        foreach ($ingredients as $i => $ing_id) {
            ...
            if ($ing_id > 0) {
                $conn->query("INSERT INTO product_ingredients (product_id, inventory_id, quantity)
                    VALUES ($product_id, $ing_id, $qty)");
            }
        }

        header('Location: /pos-system/modules/inventory/products.php?success=1');
        exit;
    }
}
...
// Fetch existing products for bundle selection
$products_query = $conn->query("SELECT m.id, m.name, c.name as category_name FROM menu_items m LEFT JOIN categories c ON m.category_id = c.id WHERE m.is_available = 1 ORDER BY m.name ASC");
...
<head>
    ...
    <title>Add New Product - Eggsy Admin</title>
            ...
            <div class="max-w-3xl mx-auto">
                ...
                <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm overflow-hidden">
                    <div class="p-8 border-b border-slate-100 dark:border-zinc-800">
                        <h2 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Add New Product</h2>
                        <p class="text-slate-500 dark:text-zinc-400 text-sm mt-1">Fill in the details to create a new product in the system.</p>
                    </div>
                    ...
                    <form class="p-8 space-y-8" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex flex-col md:flex-row items-center gap-6 p-6 border-2 border-dashed border-slate-200 dark:border-zinc-700 rounded-2xl bg-slate-50/50 dark:bg-zinc-800/50 hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors cursor-pointer group"
                                ...
                                <div class="size-32 rounded-xl bg-slate-100 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 flex flex-col items-center justify-center text-slate-400 group-hover:text-primary transition-colors overflow-hidden" id="image-preview-container">
                                    <span class="material-symbols-outlined text-4xl mb-1" id="preview-icon">image</span>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="col-span-2">
                                ...
                                <input class="w-full px-4 py-3 rounded-xl border-slate-200 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white focus:border-primary focus:ring-primary/20 transition-all text-sm"
                                    name="name" placeholder="e.g. Classic Egg Drop Sandwich" type="text"
                        ...
                        <div class="pt-6 border-t border-slate-100 dark:border-zinc-800" id="ingredients-section">
                            ...
                            <p class="text-xs text-slate-400 mt-3" id="no-ingredients-msg">No ingredients added yet. Click "Add Ingredient" to start.</p>
                        </div>
...
<script>
    ...
    function addBundleProductRow(prodId = '') {
        ...
        row.innerHTML = `
            ...
            <div class="w-36">
                <div class="relative">
                    <input name="quantity[]" class="w-full pl-3 pr-12 py-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-lg text-sm text-center focus:ring-primary/20 focus:border-primary dark:text-white" type="number" min="1" step="1" value="1"/>
